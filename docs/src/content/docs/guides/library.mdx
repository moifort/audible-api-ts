...
const { items } = await fetchCatalog(credentials, {
  ...
  pages: 3,                     // multi-page for comprehensive popularity ranking
  numResults: 20,
})
```
...
| `sortBy` | `string` | `'Relevance'` | `'BestSellers'`, `'AvgRating'`, `'Relevance'`, `'ReleaseDate'`, `'-ReleaseDate'`, `'Title'` |
| `pages` | `number` | `1` | Number of pages to fetch (max 5). When > 1, fetches across multiple sort orders for comprehensive results |
