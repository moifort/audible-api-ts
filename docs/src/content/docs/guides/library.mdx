...
const { items } = await fetchCatalog(credentials, {
  ...
  numResults: 20,               // MostVoted is the default — fetches all pages, sorts by votes
})
```

Results are automatically sorted by number of ratings (desc) then average rating (desc).
...
| `sortBy` | `string` | `'MostVoted'` | `'MostVoted'` (fetches all pages, sorts by votes), `'BestSellers'`, `'AvgRating'`, `'Relevance'`, `'ReleaseDate'`, `'-ReleaseDate'`, `'Title'` |
| `maxPages` | `number` | `10` | Max pages to fetch for `MostVoted` sort (50 items/page) |
| `keywords` | `string` | — | Search term |
| `author` | `string` | — | Filter by author name |
| `narrator` | `string` | — | Filter by narrator name |
| `numResults` | `number` | `50` | Number of results to return (max 50) |
| `page` | `number` | `1` | Page number (for single-page mode) |
