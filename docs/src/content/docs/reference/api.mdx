...
| `options.pages` | `number` | `1` | Pages to fetch (max 5). When > 1, fetches across multiple sort orders |
...
| `options.numResults` | `number` | `50` | Number of results to return (max 50) |
| `options.page` | `number` | `1` | Page number (single-page mode) |
