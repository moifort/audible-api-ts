...
type CatalogOptions = {
  ...
  sortBy?:
    | 'MostVoted'          // default — fetches all pages, sorts by votes
    | 'BestSellers'        // Audible API sort (single page)
    | 'AvgRating'
    | 'Relevance'
    | 'ReleaseDate'
    | '-ReleaseDate'
    | 'Title'
  maxPages?: number        // max pages for MostVoted (default 10)
  keywords?: string
  author?: string
  narrator?: string
  numResults?: number      // default 50
  page?: number            // page number (for non-MostVoted sorts)
}
```
...
// Latest releases (Audible API sort, single page)
await fetchCatalog(credentials, {
  category: 'science-fiction',
  sortBy: '-ReleaseDate',
})
...
const { items } = await fetchCatalog(credentials, { category: 'thriller' })
```
