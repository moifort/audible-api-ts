...
Where the reader last stopped in a library item, returned by `lastPositions()`.

```typescript
type LastPosition = {
  asin: string
  positionMs: number      // Milliseconds from the start of the recording
  lastUpdatedAt: Date     // When the player last saved the position, in UTC
}
```
...
// positions[0].positionMs      → 8275705 (2h18 in)
// positions[0].lastUpdatedAt   → 2026-09-16T20:55:15.357Z
```
...
Listening progress for a library item, as the library endpoint reports it.

<Aside type="caution">
  `percentComplete` and `timeRemainingSeconds` are often stale — a title two hours in can report 0%. For where the reader really stands, use [`lastPositions`](/audible-api-ts/reference/api/#lastpositionscredentials-asins).
</Aside>
...
type AudibleGenre =
  ...
  // Children and Young Adult sub-genres
  | 'children/fantasy'
  | 'children/science-fiction'
  | 'young-adult/fantasy'
  | 'young-adult/science-fiction'
  // …and more: see the Catalog guide for the full list
```
